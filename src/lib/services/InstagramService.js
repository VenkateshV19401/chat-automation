...
async function getUsableAccessToken(user) {
  if (!user?.accessToken) throw new Error("Missing Instagram access token");

  if (!shouldRefreshToken(user)) return { accessToken: user.accessToken, user };
  ...
  try {
    ...
    return { accessToken: updatedUser?.accessToken || user.accessToken, user: updatedUser || user };
  } catch (error) {
...
export const InstagramService = {
  async getAccountMedia(user) {
    try {
      ...
      console.log("[instagram] Loaded media:", { username: user.username, count: hydratedMedia.length });
